#!/bin/bash
# CentOS/RHEL/Fedora 主流程：更新、依赖、葵崽主流程安装
...
log_info "正在更新系统..."
(command -v dnf &>/dev/null && dnf update -y || yum update -y) || { log_error "系统更新失败"; exit 1; }

run_yq
for pkg in git wget tar xz jq epel-release redis fontconfig wqy-zenhei; do install_package "$pkg"; done
